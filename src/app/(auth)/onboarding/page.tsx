...
const LoginGridPhoto = dynamic(
  () => import("@/components/login/login-grid-photo"),
  { loading: () => <AppLoader /> }
);
const FormOnboardingPage = dynamic(
...
export default async function OnboardingPag() {
  ...
  const user = await getUserByIdServer<UserEditable>(
    userId,
    UserSchemas.Editable
  );
